...
for f in _tedseq_tmp/_sam.*S.tmp; do
	...
	samtools view -Sb _tedseq_tmp/_sam2.$outsuf |samtools sort -@ 8 > _tedseq_tmp/_bam.$outsuf
done

# Generate bedgraph files
for f in _tedseq_tmp/_bam.*S.tmp; do
	outsuf=${f#_tedseq_tmp/_bam.*}
	bedtools genomecov -ibam _tedseq_tmp/_bam.$outsuf -dz -5 -strand + |awk '{print $1"\t"$2"\t"$2+1"\t"$3}' > _tedseq_tmp/_bg.pl.$outsuf
	bedtools genomecov -ibam _tedseq_tmp/_bam.$outsuf -dz -5 -strand - |awk '{print $1"\t"$2"\t"$2+1"\t"$3}' > _tedseq_tmp/_bg.mn.$outsuf
done
...
for f in _tedseq_tmp/_bg.pl.*S.tmp; do
	...
	if [ -s _tedseq_tmp/_bg.pl.$outsuf ] && [ -s _tedseq_tmp/_bed.500.pl.tmp ]; then
		sort -k1,1 -k2,2n _tedseq_tmp/_bg.pl.$outsuf >tmp.tmp &
wait
		cat tmp.tmp> _tedseq_tmp/_bg.pl.$outsuf &
wait
		bash ./bgmatrix2.sh -a _tedseq_tmp/_bed.500.pl.tmp -b _tedseq_tmp/_bg.pl.$outsuf -split -bin 1 > _tedseq_tmp/_mat.pl.$outsuf
	fi
	if [ -s _tedseq_tmp/_bg.mn.$outsuf ] && [ -s _tedseq_tmp/_bed.500.mn.tmp ]; then
		sort -k1,1 -k2,2n _tedseq_tmp/_bg.mn.$outsuf>tmp.tmp &
wait
		cat tmp.tmp>_tedseq_tmp/_bg.mn.$outsuf &
wait
		bash bgmatrix2.sh -a _tedseq_tmp/_bed.500.mn.tmp -b _tedseq_tmp/_bg.mn.$outsuf -split -bin 1 > _tedseq_tmp/_mat.mn.$outsuf
	fi
done
