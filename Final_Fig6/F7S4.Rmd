...
```{R}
setwd("~/Desktop/Publication_Mar/Fig7/Tmp4")
...
auPos = read.table("custom3UTR.AUposition.n30649.txt", header = F, stringsAsFactors = F)
...
auPos=auPos%>%select(-chr,-interval)

head(auPos)
#PAL
dpal=read.table("custom.meanPAL.acrossalltimepoints6.27.2020.txt",header=T,stringsAsFactors = F)
```

```{R}
dpal=dpal%>%mutate(d_0_4=X4-X2)
...
#auPos.palup = auPos.cm[dpal.acm$d_0_4 >5, -(1:2)]
#auPos.palup[auPos.palup < 0] = NA
#auPos.paldn = auPos.cm[dpal.acm$d_0_4 < (-5) , -(1:2)]
#auPos.paldn[auPos.paldn < 0] = NA

auPos.palup = auPos.cm[scale(dpal.acm$d_0_4)[,1] > 1.3 , -(1:2)]
auPos.palup[auPos.palup < 0] = NA
auPos.paldn = auPos.cm[scale(dpal.acm$d_0_4)[,1] < (-1.3) , -(1:2)]
auPos.paldn[auPos.paldn < 0] = NA
...
pdf("AU_positionheatmap.2.4.ver2.pdf", width = 5, height = 3)
par(mar = c(2,2,1,1), mgp = c(1.5,0.5,0))
col = yk.col(46) 
br = 35:80/100
grid.arrange(
...
col = yk.col(10)[c(3,8)] 
pdf("MeanAUcontentperposition.2.4.pdf",width=4,height=2.5)
ggplot(df_summary%>%filter(palgroup%in%c("PAL UP","PAL DN")), aes(x=position, y=mean)) +
  ...
  ylim(c(0.1,0.9))+xlim(c(-5200,0))+labs(title="2 h vs 4 h")+
...
kmer.raw=read.table("customutr3.n.30649.kmer.txt",header=T,stringsAsFactors = F)
#head(kmer.raw)
...
pdf("6mer_enrichment.1.4.pdf", width = 5, height = 4.5)
par(mar = c(2,2,1,1), mgp = c(1.5,0.5,0))
col = yk.col(5)[2:4]
col = yk.col(7)[1:7]
ggplot(data = kmer.ratio, aes(x = ratio, y = -log10(p),size=-log10(fdr))) +
  ...
  #scale_size(guide='none')+
	...
	labs(title = "6-mer enrichment: 1 h vs 4 h", col = "n(AU) in 6mer")
