---
title: "F5S10_RBP Enrichment/Depletion in cytoplasmiclaly polyadenylated genes."
author: "Yeonui+Kwak"
date: "6/22/2020"
...
```{R}
setwd("~/Desktop/Publication_Mar/Fig7/Tmp6")
...
```{R}
getwd()
setwd("/Users/yeonuikwak/Desktop/Publication_Mar/Fig7/Tmp6")
```
...
r2.scan <- rbpbs.scanRBPsite( tfs2, "custom3UTR.n.30649.fa", 	 return.type="matches", ncores = 1 , threshold=6); 
...
utr<-read.table("custom3UTR.n.30649.fa",stringsAsFactors = F)
utr.id=utr%>%filter(grepl(">",V1))%>%separate(V1,c("newid","interval"),sep="::",remove = F)%>%
  separate(newid,c("id","ref","chr","cps"),sep=":")%>%unite("newid",c(id,ref,cps),sep=":")%>%
  #select(newid,V1)%>%
#%>%select(-interval,-chr)%>%
mutate(newid=substr(newid,2,nchar(newid)))
head(utr.id)
...
x<-read.table("custom.meanPAL.acrossalltimepoints6.27.2020.txt",header=T,stringsAsFactors = F)
...
PAL: up VS dn+nc
```{R}
gt6=read.table("0.1h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
long=bg%>%
  #inner_join(long,by="newid")%>%
  inner_join(gt6.up,by="newid")
nrow(long) #17
idx=match(long$V1,utr$V1)
...
gt6.ctrl=gt6%>%filter(dTED_group %in% c("DN","NC"))%>%select(newid)
#bg2=bg%>%filter(!(V1 %in% unlist(long$V1)))%>%
ctrl=bg%>%
  inner_join(gt6.ctrl,by="newid")
idx=match(ctrl$V1,utr$V1)
length(idx)
neg=c() 
...
t.xcomp2=t.xcomp
t.xcomp2$result=t.xcomp2$result%>%arrange(motif.id,rbp.name)%>%distinct()

#library(ggrepel)

df.ret <- t.xcomp2$result[,c('motif.id','rbp.name', 'fe.ratio', 'pvalue','pv.adj' ) ];
	df.ret$y.log <- -log10( df.ret$pvalue );
...
pdf("RBP_enrichment.Nosample.0h.1h.PALincrease.DNandNC.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  ...
  geom_point(color = dplyr::case_when(abs(df.ret$adj.y.log) >1.0~ "#EE7447", 
                                      abs(df.ret$adj.y.log)<=1.0 ~ "black"), 
             size = 0.5, alpha = 0.8) +theme_bw()+
  geom_text_repel(data        = subset(df.ret, abs(y.log)>2),
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.nosampled.0-1h.PAL up vs DNandNC.pdf", 
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_nosampled_RBP_0-1h up vs dn.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (0-1 h: PAL UP/(DN+NC))", height =7,width=7,
                   #y.max=5,
                   top.motif.labels=5,
                   bottom.motif.labels =5,
                   #color.scheme=3,
...
#0 vs 2 h
gt6=read.table("0.2h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.Nosample.0h.2h.PALincrease.DNandNC.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
  ...
  geom_text_repel(data        = subset(df.ret, abs(y.log)>2),
                  ...
                  segment.size  = 0.2,
                  segment.color = "grey50",
                  direction     = "both")+xlab("Rank order")+labs(title = expression("0h"%->%"2h"))+ylab("-log10(p-value)")+theme(legend.position = "right")+xlim(c(0,200))#+ylim(c(-3,16))
 dev.off() 


rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.nosampled.0-2h.PAL up vs DNandNC.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_nosampled_RBP_0-2h up vs dn.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (0-2 h: PAL UP/(DN+NC))", height =7,width=7,
                   #y.max=5,
                   top.motif.labels=5,
                   bottom.motif.labels =5,
                   #color.scheme=3,
...
#0 - 4 h
gt6=read.table("0.4h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.Nosample.0h.4h.PALincrease.DNandNC.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
  ...
  geom_text_repel(data        = subset(df.ret, abs(y.log)>2),
                  ...
                  segment.size  = 0.2,
                  segment.color = "grey50",
                  direction     = "both")+xlab("Rank order")+labs(title = expression("0h"%->%"4h"))+ylab("-log10(p-value)")+theme(legend.position = "right")+xlim(c(0,200))#+ylim(c(-3,16))
 dev.off() 


rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.nosampled.0-4h.PAL up vs DNandNC.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_nosampled_RBP_0-4h up vs DNandNC.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (0-4 h: PAL UP/(DN+NC))", height =7,width=7,
                   #y.max=5,
                   top.motif.labels=5,
                   bottom.motif.labels =5,
                   #color.scheme=3,
...
gt6=read.table("1.2h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.Nosample.1h.2h.PALincrease.DNandNC.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
  ...
  geom_text_repel(data        = subset(df.ret, abs(y.log)>2),
                  ...
                  segment.size  = 0.2,
                  segment.color = "grey50",
                  direction     = "both")+xlab("Rank order")+labs(title = expression("1h"%->%"2h"))+ylab("-log10(p-value)")+theme(legend.position = "right")+xlim(c(0,200))#+ylim(c(-3,16))
 dev.off() 


rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.nosampled.1-2h.PAL up vs DNandNC.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_nosampled_RBP_1-2h up vs DNandNC.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (1-2 h: PAL UP/(DN+NC))", height =7,width=7,
                   #y.max=5,
                   top.motif.labels=5,
                   bottom.motif.labels =5,
                   #color.scheme=3,
...
gt6=read.table("1.4h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.Nosample.1h.4h.PALincrease.DNandNC.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
  geom_point(color = dplyr::case_when(abs(df.ret$adj.y.log) >1.0~ "#EE7447", 
                                      abs(df.ret$adj.y.log)<=1.0 ~ "black"), 
             size = 0.5, alpha = 0.8) +theme_bw()+
  geom_text_repel(data        = subset(df.ret, abs(adj.y.log)>2),
                  #nudge_y       = -(5 - subset(df.ret, abs(y.log)>2)$y.log),
                  #nudge_x       = 150 - subset(df.ret, abs(adj.y.log)> 4)$order,
                  #nudge_x = 4.5 - subset(df.ret, abs(adj.y.log)> 4)$y.log,
                  size          = 3,
                  ...
                  segment.size  = 0.2,
                  segment.color = "grey50",
                  direction     = "both")+xlab("Rank order")+labs(title = expression("1h"%->%"4h"))+ylab("-log10(p-value)")+theme(legend.position = "right")+xlim(c(-10,230))+ylim(c(-10,23))
 dev.off() 


rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.nosampled.1-4h.PAL up vs DNandNC.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_nosampled_RBP_1-4h up vs DNandNC.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (1-4 h: PAL UP/(DN+NC))", height =7,width=7,
                   #y.max=5,
                   top.motif.labels=5,
                   bottom.motif.labels =5,
                   #color.scheme=3,
...
gt6=read.table("2.4h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.Nosample.2h.4h.PALincrease.DNandNC.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
  ...
  geom_text_repel(data        = subset(df.ret, abs(adj.y.log)>2),
                  #nudge_y       = -(5 - subset(df.ret, abs(y.log)>2)$y.log),
                  #nudge_x       = 50- subset(df.ret, abs(adj.y.log)> 2)$order,
                  #nudge_x = 4.5 - subset(df.ret, abs(adj.y.log)> 4)$y.log,
                  size          = 2.5,
                  #seed=40,
                  min.segment.length = 0,
                  ...
                  segment.size  = 0.2,
                  segment.color = "grey50",
                  direction     = "both")+xlab("Rank order")+labs(title = expression("2h"%->%"4h"))+ylab("-log10(p-value)")+theme(legend.position = "right")+xlim(c(0,200))+ylim(c(-10,23))
 dev.off() 


rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.nosampled.2-4h.PAL up vs DNandNC.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_nosampled_RBP_2-4h up vs DNandNC.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (2-4 h: PAL UP/(DN+NC))", height =7,width=7,
                   #y.max=5,
                   top.motif.labels=5,
                   bottom.motif.labels =5,
                   #color.scheme=3,
...
gt6.ctrl=gt6%>%filter(dTED_group %in% c("DN"))%>%select(newid)
#bg2=bg%>%filter(!(V1 %in% unlist(long$V1)))%>%
ctrl=bg%>%
  inner_join(gt6.ctrl,by="newid")
idx=match(ctrl$V1,utr$V1)
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.Nosample.0h.1h.PALincrease.DN.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.nosampled.0-1h.PAL up vs DN.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_nosampled_RBP_0-1h up vs DN.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (0-1 h: PAL UP/DN)", height =7,width=7,
                   #y.max=5,
                   top.motif.labels=5,
                   bottom.motif.labels =5,
                   #color.scheme=3,
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.Nosample.0h.2h.PALincrease.DN.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.nosampled.0-2h.PAL up vs DN.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_nosampled_RBP_0-2h up vs dn.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (0-2 h: PAL UP/DN)", height =7,width=7,
                   #y.max=5,
                   top.motif.labels=5,
                   bottom.motif.labels =5,
                   #color.scheme=3,
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.Nosample.0h.4h.PALincrease.DN.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.nosampled.0-4h.PAL up vs DN.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_nosampled_RBP_0-4h up vs DN.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (0-4 h: PAL UP/DN)", height =7,width=7,
                   #y.max=5,
                   top.motif.labels=5,
                   bottom.motif.labels =5,
                   #color.scheme=3,
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.Nosample.1h.2h.PALincrease.DN.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.nosampled.1-2h.PAL up vs DN.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_nosampled_RBP_1-2h up vs DN.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (1-2 h: PAL UP/DN", height =7,width=7,
                   #y.max=5,
                   top.motif.labels=5,
                   bottom.motif.labels =5,
                   #color.scheme=3,
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.Nosample.1h.4h.PALincrease.DN.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.nosampled.1-4h.PAL up vs DN.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_nosampled_RBP_1-4h up vs DN.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (1-4 h: PAL UP/(DN))", height =7,width=7,
                   #y.max=5,
                   top.motif.labels=5,
                   bottom.motif.labels =5,
                   #color.scheme=3,
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.Nosample.2h.4h.PALincrease.DN.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.nosampled.2-4h.PAL up vs DN.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_nosampled_RBP_2-4h up vs DN.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (2-4 h: PAL UP/(DN))", height =7,width=7,
                   #y.max=5,
                   top.motif.labels=5,
                   bottom.motif.labels =5,
                   #color.scheme=3,
                   ...
                   abline=2));
```
Sampling evenly distributed dTXN among dPAL groups

PAL:up vs dn+nc
sampled_0.1h.PALchange.txt
```{R}
gt6=read.table("sampled_0.1h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.sampled.0h.1h.PALincrease.DNandNC.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.sampled.0-1h.PAL up vs DNandNC.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_sampled_RBP_0-1h UP vs DNandNC.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (0-1 h: PAL UP/(DN+NC))", height =7,width=7,
                   #y.max=5,
...
#0 vs 2 h
gt6=read.table("sampled_0.2h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.sample.0h.2h.PALincrease.DNandNC.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.sampled.0-2h.PAL up vs DNandNC.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_sampled_RBP_0-2h up vs DNandNC.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (0-2 h: PAL UP/(DN+NC))", height =7,width=7,
                   #y.max=5,
...
#0 - 4 h
gt6=read.table("sampled_0.4h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.sample.0h.4h.PALincrease.DNandNC.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.sampled.0-4h.PAL up vs DNandNC.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_sampled_RBP_0-4h up vs DNandNC.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (0-4 h: PAL UP/(DN+NC))", height =7,width=7,
                   #y.max=5,
...
gt6=read.table("sampled_1.2h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.sample.1h.2h.PALincrease.DNandNC.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.sampled.1-2h.PAL up vs DNandNC.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_sampled_RBP_1-2h up vs DNandNC.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (1-2 h: PAL UP/(DN+NC))", height =7,width=7,
                   #y.max=5,
...
gt6=read.table("sampled_1.4h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.sample.1h.4h.PALincrease.DNandNC.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.sampled.1-4h.PAL up vs DNandNC.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_sampled_RBP_1-4h up vs DNandNC.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (1-4 h: PAL UP/(DN+NC))", height =7,width=7,
                   #y.max=5,
...
gt6=read.table("sampled_2.4h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.sample.2h.4h.PALincrease.DNandNC.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.sampled.2-4h.PAL up vs DNandNC.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_sampled_RBP_2-4h up vs DNandNC.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (2-4 h: PAL UP/(DN+NC))", height =7,width=7,
                   #y.max=5,
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.sampled.0h.1h.PALincrease.DN.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.sampled.0-1h.PAL up vs DN.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_sampled_RBP_0-1h UP vs DN.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (0-1 h: PAL UP/DN)", height =7,width=7,
                   #y.max=5,
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.sample.0h.2h.PALincrease.DN.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.sampled.0-2h.PAL up vs DN.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_sampled_RBP_0-2h up vs DN.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (0-2 h: PAL UP/DN)", height =7,width=7,
                   #y.max=5,
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.sample.0h.4h.PALincrease.DN.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.sampled.0-4h.PAL up vs DN.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_sampled_RBP_0-4h up vs DN.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (0-4 h: PAL UP/DN)", height =7,width=7,
                   #y.max=5,
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.sample.1h.2h.PALincrease.DN.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.sampled.1-2h.PAL up vs DN.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_sampled_RBP_1-2h up vs DN.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (1-2 h: PAL UP/DN)", height =7,width=7,
                   #y.max=5,
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.sample.1h.4h.PALincrease.DN.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.sampled.1-4h.PAL up vs DN.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_sampled_RBP_1-4h up vs DN.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (1-4 h: PAL UP/DN)", height =7,width=7,
                   #y.max=5,
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.sample.2h.4h.PALincrease.DN.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.sampled.2-4h.PAL up vs DN.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_sampled_RBP_2-4h up vs DN.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (2-4 h: PAL UP/DN)", height =7,width=7,
                   #y.max=5,
...
sampled_0.1h.PALchange.txt
```{R}
gt6=read.table("TxnNC.sampled_0.1h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
#0 vs 2 h
gt6=read.table("TxnNC.sampled_0.2h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
#0 - 4 h
gt6=read.table("TxnNC.sampled_0.4h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
gt6=read.table("TxnNC.sampled_1.2h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
gt6=read.table("TxnNC.sampled_1.4h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
gt6=read.table("TxnNC.sampled_2.4h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  ...
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
  geom_point(color = dplyr::case_when(abs(df.ret$adj.y.log) >2~ "#EE7447", 
                                      abs(df.ret$adj.y.log)<=2 ~ "black"), 
             size = 0.5, alpha = 0.8) +theme_bw()+
  geom_text_repel(data        = subset(df.ret, abs(y.log)>2),
                  #nudge_y       = -(5 - subset(df.ret, abs(y.log)>2)$y.log),
                  #nudge_x       = 150 - subset(df.ret, abs(adj.y.log)> 4)$order,
...
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  ...
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
  geom_point(color = dplyr::case_when(abs(df.ret$adj.y.log) >2.0~ "#EE7447", 
                                      abs(df.ret$adj.y.log)<=2.0 ~ "black"), 
             size = 0.5, alpha = 0.8) +theme_bw()+
  geom_text_repel(data        = subset(df.ret, abs(adj.y.log)>2),
                  #nudge_y       = -(5 - subset(df.ret, abs(y.log)>2)$y.log),
                  #nudge_x       = 50- subset(df.ret, abs(adj.y.log)> 2)$order,
...
```{R}
gt6=read.table("TxnNC.0.1h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
#0 vs 2 h
gt6=read.table("TxnNC.0.2h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
#0 - 4 h
gt6=read.table("TxnNC.0.4h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
gt6=read.table("TxnNC.1.2h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
gt6=read.table("TxnNC.1.4h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
gt6=read.table("TxnNC.2.4h.PALchange.txt",header=T,stringsAsFactors = F)
gt6=gt6%>%rename(newid2=newid)%>%rename(newid=newid_old)
gt6.up=gt6%>%filter(dTED_group=="UP")%>%select(newid)
nrow(gt6.up) #74
#long=read.table("ActD_PAL.increase.genelist0.1.txt",header=T,stringsAsFactors = F)
...
df.ret <- df.ret[order( df.ret$y.log, decreasing = F ),];
df.ret$order=1:nrow(df.ret)
pdf("RBP_enrichment.2h.4h.PALincrease.DNandNC.pdf",width=6,height=4.5)
ggplot(df.ret,aes(x=order,y=y.log,label=rbp.name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
...
rbpbs.reportEnrichment( tfs2, t.xcomp, 
	file.pdf="test-enrich.2-4h.PAL up vs DNandNC.pdf", 
	report.title="Significant Report", 
	sig.only=TRUE, 
	pv.threshold=0.05, 
	enrichment.type ="both",
...
tfbs.plotEnrichment(tfs2, t.xcomp2, file.pdf="plot-enrichment_both_all_RBP_2-4h up vs DNandNC.pdf", 
      enrichment.type="both",
      plot.type="polar",
      options=list(title="QQ plot (2-4 h: PAL UP/(DN+NC))", height =7,width=7,
                   #y.max=5,
