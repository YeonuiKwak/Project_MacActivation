...
```{R}
setwd("~/Desktop/Publication_Mar/Final_Fig7/Rcode/step10_Fig7G_RBP")
...
#last 500 regions
utr=utr%>%mutate(V2=ifelse(len>550,substr(V2,len-550,len-49),V2))
...
pdf("RBP_enrichment.Nosample.0h.1h.PALincrease.compared to alln6383.pdf",width=6,height=4.5)
...
rbpbs.reportEnrichment( tfs1, t.xcomp, 
	file.pdf="test-enrich.nosampled.0-1h.PAL up vs DNandNC_last500.pdf", 
...
tfbs.plotEnrichment(tfs1, t.xcomp2, file.pdf="plot-enrichment_both_all_nosampled_RBP_0-1h up vs dnandNC_last500.pdf", 
      enrichment.type="both",
      plot.type="polar",
...
#Putative binidng sites of ZFP36
```{R}
r2.scan <- rbpbs.scanRBPsite(tfs1.randem, "custom3UTR.n.31490.uppercase.fa", 	 return.type="matches", ncores = 1 , threshold=6)
#CDFplot
show(r2.scan)
write.table(r2.scan$summary,"allRBPbindingsitesin n.31490.3UTR sequence.txt",col.names = T,row.names = F,sep="\t",quote=F)
tmp=r2.scan$summary
which(tmp$RBP_Name=="DAZAP1")
...
ZFP36.bg=ZFP36%>%group_by(newid)%>%
  ...
  mutate(count=(count/utrlen)*1000)%>%
  mutate(dPAL0.1=X1-X0)%>%
  mutate(group=cut(count,breaks=seq(-1,30,2)))%>%
  mutate(group2=ifelse(count==0,0,ifelse(count<3,as.integer(count),"≥3")))%>%
  mutate(group2=factor(group2,levels = c(0,1,2,"≥3")))%>%mutate(PALchange=ifelse(dPAL0.1>10,"UP",ifelse(dPAL0.1<(-10),"DN","NC")))
#cut(ZFP36.bg$norm.count,breaks=seq(-1,1,0.01))

ZFP36.bg%>%group_by(group)%>%summarise(n(),mean=mean(dPAL0.1))
yk.col = function(n = 5, sat = 0.7, lum = 1) {
	col = c("royalblue4", "deepskyblue3", "turquoise3",
	   "grey62", "goldenrod2", "orange2", "orangered2") %>%
	col2rgb %>%
...
pdf("putative ZFP36 sites and dPAL0h.1h.pdf",width=4,height=2.4)
cdf_common +
  stat_ecdf(data = ZFP36.bg,
            aes(x = count,
                col = PALchange),
            geom = "step") +
  labs( title =expression("putative ZFP36 binding sites" ),color =expression(Delta*"PAL (0 h"%->%"1 h)")) +
  xlab("n(ZFP36 binding site)/kb")

dev.off()
ZFP36.bg%>%group_by(PALchange)%>%summarise(n())
wilcox.test(ZFP36.bg$count[ZFP36.bg$PALchange!="UP"],ZFP36.bg$count[ZFP36.bg$PALchange=="UP"])
#p-value = 3.371e-05
```

Invivo binding site enrichment
```{R}
...
head(bg)
UP=bg%>%filter(X1-X0>10)
#VERSION1
final=bg%>%dplyr::select(-seq)%>%#inner_join(len,by="newid")%>%
  mutate(dPAL0.1=X1-X0)%>%
  left_join(rbs,by="hgnc")%>%ungroup()%>%mutate(readcount = replace_na(readcount, 0))%>%distinct()%>%mutate(readcount=ifelse(readcount<0,-1*readcount,readcount))%>%mutate(change=ifelse(newid%in%UP$newid,"UP",ifelse(dPAL0.1<(-10),"DN","NC")))%>%mutate(change=factor(change,levels=c("UP","NC","DN")))
...
pdf("iCLIP_ZFP36_invivoBS_PAL increase_last500UTR.pdf",width=5,height=3)
cdf_common +
  stat_ecdf(data = final,size=1,
            aes(x = readcount,
                col = as.factor(change)),
            geom = "step") +  scale_color_manual(values =yk.col(5)[c(4,3,2)]) +
  labs( title =expression("ZFP36 iCLIP targets (BMDM upon LPS 1h)" ),color =expression(Delta*"PAL (0h"%->% "1h)")) +
  xlab("n(ZFP36 binding)")
dev.off()

final%>%group_by(change)%>%summarise(n())
final1=as.data.frame(final)%>%filter(change =="UP")
final2=as.data.frame(final)%>%filter(change!="UP")
...
final=bg%>%dplyr::select(-seq)%>%#inner_join(len,by="newid")%>%
  ...
  mutate(change=ifelse(readcount>=5,"ZFP36 target",ifelse(readcount==0,"Non-target","NA")))%>%filter(change!="NA")
final%>%filter(change=="ZFP36 target")%>%arrange(-dPAL0.1)
final%>%group_by(change)%>%summarise(n())
