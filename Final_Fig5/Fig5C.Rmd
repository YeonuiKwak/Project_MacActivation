---
title: "Fig5C_dPAL-dRNA for minimal TXN change"
author: "Yeonui+Kwak"
date: "8/23/2020"
...
```{R}
setwd("~/Desktop/Publication_Mar/Final_Fig5/Rcode/step3/")
...
tmp=final%>%ungroup()%>%
  ...
  filter(abs(PRO_4-PRO_0)<0.5&abs(PRO_1-PRO_0)<0.3&abs(PRO_2-PRO_1)<0.3&abs(PRO_4-PRO_2)<0.3)%>%
  mutate("dCPS_value"=log2((CPS_4+1)/(CPS_0+1)))%>%
  mutate("dTED_value"=TED_4-TED_0)%>%
  select(newid,starts_with("d"),contains("PRO"))%>%
...
ks.test(sample04$dTED_value[sample$dCPS_group=="UP"],sample$dTED_value[sample$dCPS_group=="DN"])

#Distribution of dTxn among TED-group

pdf("F5C_TxnNC.0h-4h.dTXN_distribution.pdf",width=4,height=3)
...
pdf("F5C_TxnNC_xaxis:0.4.pdf",width=5,height=3)
cdf_common +
...
head(sample)
col = yk.col(7)[2:7]
bar=sample%>%group_by(dCPS_group)%>%summarise(n=n(),mean=mean(dTED_value),sd=sd(dTED_value))
pdf("Fig5C_Barplot.0h-4h.dRNS_dPAL.pdf",width=2,height=2.5)
ggplot(bar,aes(x=dCPS_group,y=mean,fill=dCPS_group))+geom_col()+scale_fill_manual(name=expression(Delta*"RNA"),values=col)+
  geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd), width=0.1)+theme_bw()+
  labs(y=expression("Mean"*Delta*"PAL ± s.d.(nt)"), x = "∆RNA \n [3'-seq, log2 FC(4h/0h)]")+geom_hline(yintercept=10,linetype="dashed",alpha=0.4)+
  geom_hline(yintercept=(-10),linetype="dashed",alpha=0.4)+ggtitle("0h vs 4 h")+ theme(legend.position = "none")
dev.off()
...
```{R}
sample$group="txn:nc"
pdf("Txn:NC_RNA(0.4).pdf",width=3,height=2.5)
ggplot(sample,aes(x=group,y=dCPS_value))+geom_quasirandom(aes(fill=dCPS_group,col=dCPS_group),size=0.1)+scale_color_manual(name=expression(italic(Delta)*"RNA group"),values = c(yk.col(9)[c(2,5,8)],"black")) +scale_fill_manual(name=expression(italic(Delta)*"RNA group"),values = c(yk.col(9)[c(2,5,8)],"black"))+
  theme_bw()+
  ylab(expression(italic(Delta)*"RNA [3´seq log2 FC(4h/0h)]"))+xlab("")
dev.off()
...
tmp=final%>%ungroup()%>%
  ...
  mutate("dTED_value"=TED_4-TED_0)%>%
  select(newid,starts_with("d"),contains("PRO"))%>%
  mutate("dCPS_group"=ifelse(dCPS_value<(-1),"DN",ifelse(dCPS_value>1,"UP",ifelse(abs(dCPS_value)<0.5,"NC","N/A"))))%>%mutate(dCPS_group=factor(dCPS_group,levels=c("DN","NC","UP","N/A")))
tmp$group="Txn:NC"
par(mar = c(2,2,1,1), mgp = c(1.5,0.5,0))
#col = c ("#56B4E9","#999999","#E69F00")
pdf("Txn:NC_RNA(0.4).pdf",width=2.3,height=2)
ggplot(tmp,aes(x=group,y=dCPS_value))+geom_dotplot(aes(fill=dCPS_group,col=dCPS_group),binaxis="y", stackdir="center",method="histodot", binwidth=0.1,dotsize=1)+scale_color_manual(name=expression(italic(Delta)*"RNA group"),values = c(yk.col(9)[c(2,5,8)],"black")) +scale_fill_manual(name=expression(italic(Delta)*"RNA group"),values = c(yk.col(9)[c(2,5,8)],"black"))+
  theme_bw()+
  ylab(expression(italic(Delta)*"RNA [3´seq log2 FC(4h/0h)]"))+xlab("")
dev.off()
...
#Distribution of dTxn among TED-group
```{R}
pdf("F5C_TxnNC.dTXN_distribution_0.1.pdf",width=3.5,height=3.5)
ggplot(sample,aes(x=dCPS_group,y=dPRO,col=dPRO_bin))+geom_quasirandom(size=0.2)+scale_colour_manual(values=yk.col(10)[1:10])+ylab(expression(italic(Delta)*"TXN (PRO-seq log2 FC)"))+labs(title=expression("0h"%->%"1h"))+theme_bw()+xlab(expression(italic(Delta)*"RNA"))+labs(col=expression(italic(Delta)*"TXN"))+ylim(c(-1,1))
dev.off()
```
5. Set CDF color (N=3)
```{R}
