...
```{R}
setwd("~/Desktop/Publication_Mar/Fig5/Tmp4")
...
sample=all%>%select(c(starts_with("PRO"),starts_with("CPS"),TED_0,TED_4,id))%>%
  ...
  mutate("dCPS_0and4"=CPS_4-CPS_0)%>%mutate("dCPS_2and4"=CPS_4-CPS_2)%>%mutate("dCPS_1and4"=CPS_4-CPS_1)%>%
  mutate("dTED_0and4"=TED_4-TED_0)%>%
  select(id,starts_with("d"))%>%
  mutate(dPROgroup=ifelse((dPRO_0and4>0.5&dPRO_0and1>0&dPRO_1and2>(0)&dPRO_2and4>(0)),"UP",ifelse((dPRO_0and4<=(-1)&dPRO_0and4>(-2)&dPRO_0and1<0& dPRO_1and2<=0&dPRO_2and4<=0),"DN",ifelse(abs(dPRO_0and1)<0.5&abs(dPRO_0and2)<(0.5)&abs(dPRO_0and4)<(0.5),"NC","NA"))))%>%
  filter(dPROgroup!="NA")%>%
  ...
  mutate("dTED_group"=ifelse(dTED_0and4<(-15),"PAL_DN",ifelse(dTED_0and4>(10),"PAL_UP",ifelse(abs(dTED_0and4)<5,"PAL_NC","NA"))))%>%
  filter(dTED_group!="NA")%>%
  gather("dCPS_diff","dCPS_value",starts_with("dCPS"))%>%filter(dCPS_diff=="dCPS_2and4")%>%filter(dTED_group!="PAL_UP")
...
sample$dTED_group%>%table
```


```{R}
t=sample%>%group_by(dTED_group)%>%count
cat("sample number for no change:",t[t$dTED_group=="PAL_NC",]$n,", sample number for DN:",t[t$dTED_group=="PAL_DN",]$n)
pdf("F5B_2.pdf",width=4,height=2.5)
ggplot(sample,aes(x=dTED_group,y=dCPS_value))+ geom_quasirandom(aes(col=dTED_group),size=1)+scale_colour_manual(values=col )+theme_bw()+ylab(expression(italic(Delta)*"RNA(log2 FC (4hr/2hr))"))+geom_boxplot(alpha=0.3, color="black", width=.2)+xlab(expression(italic(Delta)*"PAL"))+ggtitle("Genes with transcription down")+scale_color_manual(labels = c("DN","NC","UP"),values=col)+
...
```{R}
pdf("F5B_5.pdf",width=4,height=2.5)
ggplot(sample,aes(x=dTED_group,y=dPRO_0and4))+ geom_quasirandom(aes(col=dTED_group),size=1)+scale_colour_manual(values=col )+theme_bw()+ylab(expression(italic(Delta)*"Txn(log2 FC (4hr/0hr))"))+geom_boxplot(alpha=0.3, color="black", width=.2)+xlab(expression(italic(Delta)*"PAL"))+ggtitle("Genes with transcription down")+scale_color_manual(labels = c("DN","NC","UP"),values=col)+
  scale_x_discrete(labels=c("PAL_DN" = "DN", "PAL_NC" = "NC","PAL_UP" = "UP"))+labs(col=expression(italic(Delta)*"PAL"))
#+geom_beeswarm(size=2,aes(col=dTED_group))
dev.off()
...
res.aov <- aov(dCPS_value ~dTED_group, data = sample)
res.aov <- aov(dPRO_0and4~dTED_group, data = sample)
...
```{R}
pdf("F5A.pdf",width=4.5,height=2.3)
...
sample=all%>%select(c(starts_with("PRO"),starts_with("CPS"),TED_0,TED_4,id))%>%
  ...
  mutate("dTED_0and4"=TED_4-TED_0)%>%
  select(id,starts_with("d"))%>%
  mutate(dPROgroup=ifelse((dPRO_0and4>0.5&dPRO_0and1>0&dPRO_1and2>(0)&dPRO_2and4>(0)),"UP",ifelse((dPRO_0and4<=(-1)&dPRO_0and4>(-2)&dPRO_0and4>(-1.5)&dPRO_0and1<0& dPRO_1and2<0&dPRO_2and4<0),"DN",ifelse(abs(dPRO_0and1)<0.5&abs(dPRO_0and2)<(0.5)&abs(dPRO_0and4)<(0.5),"NC","NA"))))%>%
  filter(dPROgroup!="NA")%>%
  mutate(dPROgroup=factor(dPROgroup,levels=c("DN","NC","UP")))%>%
  filter(dPROgroup=="DN")%>%
  #mutate(dPRO_quantile=ifelse(dPRO_0and1<quantile(dPRO_0and1,0.25),"25",ifelse(dPRO_0and1<quantile(dPRO_0and1,0.5),"50",ifelse(dPRO_0and1<quantile(dPRO_0and1,0.75),"75","100"))))%>%filter(dPRO_quantile!="100" &dPRO_quantile!="75")%>%
  #mutate("dTED_group"=ifelse(dTED_0and4<(-10),"PAL_DN",ifelse(dTED_0and4>(10),"PAL_UP",ifelse(abs(dTED_0and4)<5,"PAL_NC","NA"))))%>%
  mutate("dTED_group"=ifelse(dTED_0and4<(-15),"PAL_DN",ifelse(dTED_0and4>(10),"PAL_UP",ifelse(abs(dTED_0and4)<5,"PAL_NC","NA"))))%>%
  filter(dTED_group!="NA"&dTED_group!="PAL_UP")
...
pdf("F5C.pdf",width=4.5,height=2.3)
pal.labs <- c("PAL : DN", "PAL : NC", "PAL : UP"); names(pal.labs) <- c("PAL_DN", "PAL_NC","PAL_UP")
ggplot(df_summary, aes(x=hour, y=mean)) +xlab("Time upon LPS stimulation")+
...
pdf("F5D.pdf",width=4.5,height=2.3)
pal.labs <- c("PAL : DN", "PAL : NC", "PAL : UP"); names(pal.labs) <- c("PAL_DN", "PAL_NC","PAL_UP")
ggplot(df_summary, aes(x=hour, y=mean)) +xlab("Time upon LPS stimulation")+
   geom_line(data=df_summary, aes(x=hour, y=mean,group=dTED_group), size=1, alpha=0.8)+geom_ribbon(aes(ymin=CI_lower, ymax=CI_upper,group=dTED_group,fill=dTED_group), alpha=0.3)+scale_fill_manual(values=col,name = "PAL", labels = c("DN", "NC", "UP"))+theme_bw()+facet_wrap(dTED_group~.,labeller = labeller(dTED_group=pal.labs))+
  scale_fill_manual(values=col,name = "PAL", labels = c("DN", "NC", "UP"))+ylab("mean (PAL) ± 95% CI")+ylim(c(-40,20))
dev.off()
