...
sample=final%>%select(c(starts_with("PRO"),starts_with("CPS"),starts_with("TED"),newid))%>%
  ...
  mutate(dPROgroup=ifelse((dPRO_0and4>1&dPRO_0and1>=0&dPRO_1and2>=0),"UP",ifelse((dPRO_0and4<(-1)&dPRO_0and1<0& dPRO_1and2<0),"DN",ifelse(abs(dPRO_0and4)<(0.5)&abs(dPRO_0and1)<(0.3)&abs(dPRO_1and2)<(0.3)&abs(dPRO_2and4)<0.3,"NC","NA"))))%>%
  mutate(dPROgroup=factor(dPROgroup,levels=c("DN","NC","UP")))%>%
  filter(dPROgroup=="UP")%>%
  mutate("dTED_group"=ifelse(dTED_1and2<(-5),"PAL_DN",ifelse(dTED_1and2>(5)&dTED_2and4>=0,"PAL_UP",ifelse(abs(dTED_1and2)<3,"PAL_NC","NA"))))%>%
  filter(dTED_group!="NA")%>%
  gather("dCPS_diff","dCPS_value",starts_with("dCPS"))%>%
  #filter(dCPS_diff=="dCPS_2and4")%>%
...
pdf("F5I.pdf",width=4.5,height=2.5)
ggplot(sample%>%filter(dCPS_diff %in% c("dCPS_1and2","dCPS_2and4")),aes(x=dTED_group,y=dCPS_value))+
  geom_quasirandom(aes(col=dTED_group),size=0.5)+facet_grid(.~dCPS_diff)+
  scale_colour_manual(name=expression(Delta*"PAL"),values=col )+theme_bw()+ylab(expression(italic(Delta)*"RNA [3´-seq log2 FC (4h/2h)]"))+
...
sample%>%filter(dCPS_diff=="dCPS_2and4")%>%group_by(dTED_group)%>%summarise(n())

test=sample%>%filter(dCPS_diff=="dCPS_2and4")
t.test(test$dCPS_value[test$dTED_group=="PAL_NC"],test$dCPS_value[test$dTED_group=="PAL_DN"])
```
...
sample_=sample%>%mutate(.PRO_0=0)%>%mutate(.PRO_1=dPRO_0and1)%>%mutate(.PRO_2=dPRO_0and2)%>%mutate(.PRO_4=dPRO_0and4)%>%select(newid,starts_with(".PRO_"),dTED_group)%>%
  gather("time","txn",starts_with(".PRO_"))%>%filter(txn>=0)%>%separate(time,c("PRO","hour"),sep="_")%>%mutate(hour=as.numeric(hour))%>%select(dTED_group,hour,txn,newid)
...
pdf("F5H_TXN.pdf",width=6,height=2.3)
...
pdf("F5H_PAL.pdf",width=6,height=2.3)
pal.labs <- c("PAL : DN", "PAL : NC", "PAL : UP"); names(pal.labs) <- c("PAL_DN", "PAL_NC","PAL_UP")
...
pdf("F5H_RNA.pdf",width=6,height=2.3)
pal.labs <- c("PAL : DN", "PAL : NC", "PAL : UP"); names(pal.labs) <- c("PAL_DN", "PAL_NC","PAL_UP")
