---
title: "Step8_ActD_association between dRNA and dPAL"
author: "Yeonui+Kwak"
date: "8/17/2020"
...
```{R}
setwd("~/Desktop/Publication_Mar/Final_Fig8/Rcode/step8")
```
...
3. Load sample
```{R}
pal<-read.table("Actd.custom.meanPAL.acrossalltimepoints.txt",header=T, stringsAsFactors = F)
pal=pal%>%dplyr::select(hgnc,newid,X0,X1,X2)
...
sampln_=final%>%
  ...
  dplyr::select(newid,starts_with("d"))%>%
  mutate("dTED_group"=ifelse(dTED_0and1>=(5),"PAL_UP",ifelse(dTED_0and1<1,"PAL_CTRL","NA")))%>%filter(dTED_group!="NA")%>%
   #binning
  ...
  ungroup()%>%dplyr::select(sum,dPRO_bin)
...
sample=sampled%>%dplyr::select(-data)%>%unnest(samp)
sample01=sample
sample%>%group_by(dTED_group)%>%summarise(n())
...
```{R}
pdf("F5_ActD_General_xaxis:dRNA_0.1pdf",width=4,height=2.5)
cdf_common +
...
sampln_=final%>%ungroup()%>%
  ...
  dplyr::select(newid,starts_with("d"))%>%
  mutate("dTED_group"=ifelse(dTED>=(5),"PAL_UP",ifelse(dTED<1,"PAL_CTRL","NA")))%>%filter(dTED_group!="NA")%>%
   #binning
  mutate(dPRO_bin=cut(dPRO,breaks=dpro.breaks))%>%
  group_by(dTED_group,dPRO_bin)%>%summarise(sum=n())%>%ungroup()%>%
  complete(dTED_group,nesting(dPRO_bin),fill =list(sum=0))%>%
...
```{R}
pdf("F5_ActD_General_xaxis:dRNA_1.2.pdf",width=4,height=2.5)
cdf_common +
  stat_ecdf(data = sample,
            aes(x = dCPS,
                col = dTED_group),
...
sampln_=final%>%
  ...
  mutate("dCPS_0and1"=log2((CPS_1+1)/(CPS_0+1)))%>%
  mutate("dTED_0and1"=TED_1-TED_0)%>%
  dplyr::select(newid,newid_old,starts_with("d"))%>%
...
sample2=final%>%
  ...
  separate("time",c("pro","prohour"),sep="_")%>%dplyr::select(-pro)%>%mutate(prohour=as.numeric(prohour))
