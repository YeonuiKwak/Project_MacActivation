...
```{R}
setwd("~/Desktop/Publication_Mar/Final_Fig8/Rcode/step7_RBP")
```
...
Function 1: CISRBP.active_motif_info definition
```{R}
##Subfunctions.
...
Function 2: rbpbs 
Function 3: rbp_creatFromCisRBP
```{r}
...
Set class of rbpbs object, and how to show rbpbs object.
```{r}
#set class rbpbs
setClass("rbpbs",
	representation(
...
Function 4:rbpbs_createFromCisRBP
```{r}
...
Function 2: CisRBP.getRBPinformation()
```{r}
...
1. Load RBP database and Create db object from pre-installed dataset.
```{R}
...
Function7:tfbs_draw_heatmap_style
...
Functions: yb.sig.pal, write.starchbed; get_chromosome_size; save_chromosome_size; get_os ; check_command_error
...
#Funciton8: rbpbs_drawLogosForClusters
```{R}
...
Function 9: rbpbs.selectByRandom
```{R}
...
#Function 10: subfucntion:scanDb_rtfbs;get_binding_site;	scan_each_motif;rbpbs_scanRBPsite;
print.tfbs.finding;summary.tfbs.finding;tfbs.reportFinding;summary.tfbs.finding;smallres_starchcat
#FUNCTION 10: rbpbs.scanRBPsite
```{r}
#scanDB
...
#Function 11: Output.motif.report
```{R}
...
#Function11. rbpbs.enrichmentTest
subfunction: rbpbs_enrichment
```{R}
...
#Function13: joint.gc.quantile.bins 
```{R}
#create GC beans
...
#Function14:comparative_scan_rtfbs.ms (Compare groupA vs groupB)
```{R}
...
#Function15:comparative_scan_rtfbs
```{R}
...
#Function 16: comparative_scanDb_rtfbs
```{R}
...
#Function 17: summary.tfbs.enrichment
```{r}
...
#Function 18: tfbs.draw.enrichment
```{R}
...
#Function 19: tfbs.plotEnrichment
```{R}
...
#Code1.Cluster the motifs with similarity
...
#Code3. Enrichment of RBP binding sites in the 3UTRs of transcripts with PAL increase.
```{R}

# Compare motifs between two conditions
utr<-read.table("custom3UTR.n.31490.uppercase.tab.fa",stringsAsFactors = F)
utr=utr%>%mutate(len=nchar(V2))
utr.id=utr%>%
  separate(V1,c("id","ref","chr","cps"),sep=":")%>%
  unite("newid2",c(id,ref,cps),sep=":",remove=F)%>%
  unite("newid",c(id,cps),sep=":",remove=F)%>%
mutate(newid=substr(newid,2,nchar(newid)-3))
#last 500 regions
utr=utr%>%mutate(V2=ifelse(len>550,substr(V2,len-550,len-49),substr(V2,1,len-49)))
...
x<-read.table("Actd.custom.meanPAL.acrossalltimepoints.txt",header=T,stringsAsFactors = F)
y=read.table("ActD_PALchange.transcriptid.list.txt",header=T,stringsAsFactors = F,sep="\t")
head(y)
bg=x%>%
...
```{R}
upgene=y%>%filter(comparison=="0h - 1h"&change=="UP")
UP=bg%>%filter(newid%in%unlist(upgene$newid))
nrow(UP)#49
head(bg)
UP=UP%>%dplyr::select(newid2,seq)#%>%mutate(newid2=paste0(">",newid2))
write.table(UP,"up.fa",col.names = F,row.names = F, sep="\n",quote=F)

CTRL=bg%>%
  filter(!(newid%in%unlist(upgene$newid)))%>%
  filter(X1-X0<(0))%>%
...
pdf("RBP_enrichment.Nosample.0h.1h.PALincrease.compared to alln7100_Last500_2.pdf",width=6,height=4.5)
...
```{R}
rbpbs.reportEnrichment( tfs1, t.xcomp, 
...
tfbs.plotEnrichment(tfs1, t.xcomp, file.pdf="plot-enrichment_both_all_nosampled_RBP_0-1h up vs dnandNC.pdf", 
      enrichment.type="both",
      plot.type="polar",
...
#Code2: Build markov model background for hg38 3´UTR sequence.
```{R}
r2.scan <- rbpbs.scanRBPsite(tfs1, "custom3UTR.n.31490.uppercase.fa", 	 return.type="matches", ncores = 6 , threshold=6)
#CDFplot
(r2.scan)
write.table(r2.scan$summary,"allRBPbindingsitesin n.31490.3UTR sequence.txt",col.names = T,row.names = F,sep="\t",quote=F)
tmp=r2.scan$summary
len=read.table("custom.UTRlen.table.txt", header=T,stringsAsFactors = F)
len=len%>%dplyr::select(V4,utrlen)%>%separate(V4,c("enst","ref","chr","cps"),sep=":")%>%unite("newid",c(enst,cps),sep=":")%>%dplyr::select(-chr)

#ZFP36=bind_rows(ZFP36.1,ZFP36.2)
```
...
for (i in c(1:202)){
  ...
  pval=c(pval,"null")
  diff=c(diff,"null")}
  else if(is.null(r2.scan$result[[i]])){
    pval=c(pval,"null")
    diff=c(diff,"null")}
  else{
    MATR3=r2.scan$result[[i]]%>%separate(seqname,c("enst","ref","chr","cps"),sep=":")%>%unite("newid",c(enst,cps),sep=":")%>%mutate(newid=substr(newid,1,nchar(newid)-3))
    ZFP36.2=r2.scan$result[[3]]%>%separate(seqname,c("enst","ref","chr","cps"),sep=":")%>%unite("newid",c(enst,cps),sep=":")%>%mutate(newid=substr(newid,1,nchar(newid)-3))
    matr3.bg=MATR3%>%group_by(newid)%>%
  summarise(count=n())%>%
  ungroup()%>%
  right_join(bg%>%dplyr::select(-seq)%>%inner_join(len,by="newid"),by="newid")%>%
  mutate(count=replace_na(count, 0))%>%
  mutate(norm.count=count)%>%
  #mutate(norm.count=(count/utrlen)*1000)%>%
  mutate(dPAL0.1=X1-X0)%>%mutate(group=cut(norm.count,breaks=seq(-1,30,2)))%>%mutate(group2=ifelse(norm.count<1,0,"≥1"))%>%mutate(group2=factor(group2,levels = c(0,"≥1")))
    zfp36.bg=ZFP36.2%>%group_by(newid)%>%
  summarise(count=n())%>%
  ungroup()%>%
  right_join(bg%>%dplyr::select(-seq)%>%inner_join(len,by="newid"),by="newid")%>%
  mutate(count=replace_na(count, 0))%>%
  ...
  #mutate(norm.count=(count/utrlen)*1000)%>%
  mutate(dPAL0.1=X1-X0)%>%mutate(group=cut(norm.count,breaks=seq(-1,30,2)))%>%mutate(group2=ifelse(norm.count<1,0,"≥1"))%>%mutate(group2=factor(group2,levels = c(0,"≥1")))
    final=zfp36.bg%>%dplyr::select(newid,group2,hgnc)%>%rename("ZFP36bindingsites"=group2)%>%inner_join(matr3.bg%>%dplyr::select(newid,group2,dPAL0.1)%>%rename("MATR3bindingsites"=group2),by="newid")
    final=final%>%mutate("newgroup"=ifelse(ZFP36bindingsites=="≥1"&MATR3bindingsites=="≥1","both",ifelse(ZFP36bindingsites=="≥1"&MATR3bindingsites!="≥1","ZFP36 only",ifelse(ZFP36bindingsites!="≥1"&MATR3bindingsites=="≥1","MATR3 only","none"))))#%>%mutate(PALgroup=ifelse(newid%in%upgene$newid,"UP",ifelse(dPAL0.1<3,"CTRL","NA")))%>%filter(PALgroup!="NA")%>%mutate(PALgroup=factor(PALgroup,levels=c("UP","CTRL")))
    final%>%filter(newgroup=="both")%>%filter(dPAL0.1>3)%>%arrange(hgnc)
    #col = yk.col(4)[c(1:5)]         
    final1=as.data.frame(final)%>%filter(newgroup =="both")
    final2=as.data.frame(final)%>%filter(newgroup=="ZFP36 only")
    if(nrow(final1)>30&nrow(final2)>30){
    pval=c(pval,wilcox.test(final1$dPAL0.1,final2$dPAL0.1)$p.value)
    diff=c(diff,mean(final1$dPAL0.1)-mean(final2$dPAL0.1))
    }else{
  pval=c(pval,1);diff=c(diff,"null")}
...
```{R}
seq(from=1,to=403,by=2)
#diff[seq(from=1,to=403,by=2)]
df=data.frame("p.value"=pval,"name"=tmp$RBP_Name,"effect"=diff)
df=as.tibble(df)%>%mutate(row=row_number())%>%filter(p.value!="null")%>%
  mutate(p.value=unlist(p.value))%>%mutate(p.value=as.character(p.value))%>%
  mutate(p.value=as.numeric(p.value))
df%>%filter(p.value<0.01)
df=df%>%mutate(expression=ifelse((df$name %in%x$hgnc),"Y","N"))%>%filter(expression=="Y")
df$adj.pval=p.adjust(df$p.value, method ="fdr")
df%>%filter(adj.pval<0.1)%>%arrange(adj.pval)
df$y.log=-log10(df$p.value)
df$adj.y.log=-log10(df$adj.pval)
df <- df[order( df$y.log, decreasing = F ),];
df$order=1:nrow(df)
pdf("RBP+ZFP36 makes more increase.pdf",width=6,height=5)
ggplot(df,aes(x=order,y=y.log,label=name))+
  #geom_point(aes(colour=fe.ratio))+
  #scale_colour_gradientn(colours = yk.col(10)[1:10])
  #scale_colour_gradient( low = "#132B43", high = "#56B1F7",space = "Lab", na.value = "grey50", guide = "colourbar")
  geom_point(color = dplyr::case_when(abs(df$adj.y.log) >1.0~ "#EE7447", 
                                      abs(df$adj.y.log)<=1.0 ~ "black"), 
             size = 1, alpha = 0.8) +theme_bw()+
  geom_text_repel(data        = subset(df, abs(y.log)>2),
                  #nudge_y       = -(5 - subset(df.ret, abs(y.log)>2)$y.log),
                  #nudge_x       = 150 - subset(df.ret, abs(adj.y.log)> 4)$order,
                  #nudge_x = 4.5 - subset(df.ret, abs(adj.y.log)> 4)$y.log,
                  size          = 6,
                  ...
                  segment.size  = 0.2,
                  segment.color = "grey50",
                  direction     = "both")+xlab("Rank order")+labs(title = expression("0h"%->%"1h"))+ylab("-log10(p-value)")+theme(legend.position = "right")+ylim(c(0,6))
 dev.off() 
...
```
```{R}
ELAVL1.1=r2.scan$result[[59]]%>%separate(seqname,c("enst","ref","chr","cps"),sep=":")%>%unite("newid",c(enst,cps),sep=":")%>%mutate(newid=substr(newid,1,nchar(newid)-3))
ELAVL1.2=r2.scan$result[[60]]%>%separate(seqname,c("enst","ref","chr","cps"),sep=":")%>%unite("newid",c(enst,cps),sep=":")%>%mutate(newid=substr(newid,1,nchar(newid)-3))
ELAVL1.3=r2.scan$result[[61]]%>%separate(seqname,c("enst","ref","chr","cps"),sep=":")%>%unite("newid",c(enst,cps),sep=":")%>%mutate(newid=substr(newid,1,nchar(newid)-3))
ELAVL1.4=r2.scan$result[[62]]%>%separate(seqname,c("enst","ref","chr","cps"),sep=":")%>%unite("newid",c(enst,cps),sep=":")%>%mutate(newid=substr(newid,1,nchar(newid)-3))
ELAVL1=bind_rows(ELAVL1.1,ELAVL1.2,ELAVL1.3,ELAVL1.4)


ZFP36.1=r2.scan$result[[2]]%>%separate(seqname,c("enst","ref","chr","cps"),sep=":")%>%unite("newid",c(enst,cps),sep=":")%>%mutate(newid=substr(newid,1,nchar(newid)-3))
ZFP36.2=r2.scan$result[[3]]%>%separate(seqname,c("enst","ref","chr","cps"),sep=":")%>%unite("newid",c(enst,cps),sep=":")%>%mutate(newid=substr(newid,1,nchar(newid)-3))
ZFP36=bind_rows(ZFP36.1,ZFP36.2)


elavl1.bg=ELAVL1%>%group_by(newid)%>%
  summarise(count=n())%>%
  ungroup()%>%
  right_join(bg%>%dplyr::select(-seq)%>%inner_join(len,by="newid"),by="newid")%>%
  mutate(count=replace_na(count, 0))%>%
...
zfp36.bg=ZFP36%>%group_by(newid)%>%
  ...
  mutate(norm.count=count)%>%
  #mutate(norm.count=(count/utrlen)*1000)%>%
  mutate(dPAL0.1=X1-X0)%>%
  mutate(group=cut(norm.count,breaks=seq(-1,30,2)))%>%
  mutate(group2=ifelse(norm.count<1,0,"≥1"))%>%mutate(group2=factor(group2,levels = c(0,"≥1")))
    
    
final=zfp36.bg%>%dplyr::select(newid,group2,hgnc)%>%rename("ZFP36bindingsites"=group2)%>%inner_join(elavl1.bg%>%dplyr::select(newid,group2,dPAL0.1)%>%rename("ELAVL1bindingsites"=group2),by="newid")
    final=final%>%mutate("newgroup"=ifelse(ZFP36bindingsites=="≥1"&ELAVL1bindingsites=="≥1","both",ifelse(ZFP36bindingsites=="≥1"&ELAVL1bindingsites!="≥1","ZFP36 only",ifelse(ZFP36bindingsites!="≥1"&ELAVL1bindingsites=="≥1","ELAVL1 only","none"))))%>%
      mutate(newgroup=factor(newgroup,level=c("none","ZFP36 only","ELAVL1 only","both")))
    #%>%mutate(PALgroup=ifelse(newid%in%upgene$newid,"UP",ifelse(dPAL0.1<3,"CTRL","NA")))%>%filter(PALgroup!="NA")%>%mutate(PALgroup=factor(PALgroup,levels=c("UP","CTRL")))
    final%>%filter(newgroup=="both")%>%filter(dPAL0.1>3)%>%arrange(hgnc)
     
    final1=as.data.frame(final)%>%filter(newgroup =="both")
    final2=as.data.frame(final)%>%filter(newgroup=="ZFP36 only") #p-value = 0.0001675 # zfp elavl2 p-value = 0.6603
    final2=as.data.frame(final)%>%filter(newgroup=="ELAVL1 only") # p-value = 4.051e-07
    final2=as.data.frame(final)%>%filter(newgroup=="none")#p-value < 2.2e-16
col = yk.col(4)[c(1:4)]    
cdf_common = ggplot() +
  scale_color_manual(values = col) +
  theme_bw() +
  xlim(-10, 10) +
  ylab("Cumulative fraction")
pdf("Combination of ELAVL1 and ZFP36 mediates increase in PAL.pdf",width=5,height=2.5)
cdf_common +
  stat_ecdf(data = final,
            aes(x = dPAL0.1,
                col = newgroup),
            geom = "step") +
  labs( title =expression("0h"%->%"1h" ),color =expression("putative RBP binding sites")) +
  xlab(expression("post-TXN "*Delta*"PAL"))
dev.off()
#cut(ZFP36.bg$norm.count,breaks=seq(-1,1,0.01))
ks.test(final1$dPAL0.1,final2$dPAL0.1)
  final%>%group_by(newgroup)%>%summarise(n())
```

Putative ZFP36 binding sites in the 3'UTR
...
pdf("ActD_ZFP36 enriched genes and dPAL0h.1h.pdf",width=5,height=2.4)
cdf_common +
  stat_ecdf(data = ZFP36.bg,
            aes(x = dPAL0.1,
                col = group2),
            geom = "step") +
  labs( title =expression("0 h vs 1 h" ),color =expression("n(putative ZFP36 binding site)")) +
  xlab(expression(Delta*"PAL"))
dev.off()

# p-value = 0.0002519
ks.test(ZFP36.bg$dPAL0.1[ZFP36.bg$group2!="≥3"],ZFP36.bg$dPAL0.1[ZFP36.bg$group2=="≥3"])
```



in vivo ZFP36 binding sites
...
upgene=y%>%filter(comparison=="0h - 1h"&change=="UP")
head(upgene)
final=bg%>%dplyr::select(-seq)%>%inner_join(len,by="newid")%>%
  mutate(dPAL0.1=X1-X0)%>%
  left_join(rbs,by="hgnc")%>%ungroup()%>%mutate(readcount = replace_na(readcount, 0))%>%distinct()%>%mutate(readcount=ifelse(readcount<0,-1*readcount,readcount))%>%mutate(change=ifelse(newid%in%upgene$newid,"UP",ifelse(dPAL0.1<3,"CTRL","NA")))%>%filter(change!="NA")%>%mutate(change=factor(change,levels=c("UP","CTRL")))
...
pdf("iCLIP_ZFP36_invivoBS_PostTxnPAL increase.pdf",width=5,height=3)
cdf_common +
  stat_ecdf(data = final,size=1,
            aes(x = readcount,
                col = as.factor(change)),
            geom = "step") +  scale_color_manual(values =yk.col(5)[c(2,5)]) +
  labs( title =expression("ZFP36 iCLIP targets (BMDM upon LPS 1h)" ),color =expression("post-TXN "*Delta*"PAL (0h"%->% "1h)")) +
  xlab("n(ZFP36 binding)")
dev.off()
final%>%filter(change=="UP")


final1=as.data.frame(final)%>%filter(change =="UP")
final2=as.data.frame(final)%>%filter(change=="CTRL")
wilcox.test(final1$readcount,final2$readcount)
...
final=bg%>%dplyr::select(-seq)%>%#inner_join(len,by="newid")%>%
  mutate(dPAL0.1=X1-X0)%>%
  left_join(rbs,by="hgnc")%>%ungroup()%>%mutate(readcount = replace_na(readcount, 0))%>%distinct()%>%mutate(readcount=ifelse(readcount<0,-1*readcount,readcount))%>%
  mutate(change=ifelse(readcount>=1,"ZFP Target","Non-target"))#%>%na.omit
final%>%filter(readcount==0)
final%>%group_by(change)%>%summarise(n())
pdf("iCLIP_ZFP36_invivoBS_PostTxnPAL increase2.pdf",width=7,height=3)
cdf_common +
  stat_ecdf(data = final,size=1,
            aes(x = dPAL0.1,
                col = as.factor(change)),
            geom = "step") +  scale_color_manual(values =yk.col(5)[c(2,5)]) +
  labs(color =expression("in vivo iCLIP targets (BMDM upon LPS 1h)" )) +
  xlab(expression("post-TXN "*Delta*"PAL (0h"%->% "1h)"))

dev.off()

final1=as.data.frame(final)%>%filter(change =="ZFP Target")
final2=as.data.frame(final)%>%filter(change=="Non-target")
wilcox.test(final1$dPAL0.1,final2$dPAL0.1)
