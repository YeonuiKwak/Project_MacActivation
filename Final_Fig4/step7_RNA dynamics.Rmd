...
pdf("F5_TXNdynamicsBYcluster_n1520_linecol8_2.pdf",height=8,width=2.5)
tmp2=tmp%>%gather("time","txn",-PASid,-newcl,-hgnc)%>%
  mutate(time=as.numeric(time))%>%group_by(PASid)%>%ungroup()
ggplot(tmp2,aes(time,txn))+
  geom_line(aes(col=as.factor(newcl),group=PASid,alpha=0.2))+facet_grid(newcl~.,scales="fixed",switch="both")+
  geom_line(stat = 'summary', fun = 'mean', size = .5, color = 'black',linetype="dashed") +
  ylab("TXN")+
...
head(proseq)
head(highly_Var)
tmp=highly_Var%>%inner_join(proseq%>%select(hgnc,contains("CPS")),by="hgnc")%>%
  group_by(PASid,hgnc,newcl,`0`,`1`,`2`,`4`)%>%summarise(PRO0=log2(mean(CPS_0)/mean(CPS_0)),PRO1=log2(mean(CPS_1)/mean(CPS_0)),PRO2=log2(mean(CPS_2)/mean(CPS_0)),PRO4=log2(mean(CPS_4)/mean(CPS_0)))
tmp=tmp%>%ungroup()%>%select(PRO0,PRO1,PRO2,PRO4,hgnc,newcl,PASid)
tmp%>%filter(hgnc=="AAGAB")
ha = HeatmapAnnotation(df = data.frame(TimeuponLPS= as.factor(c(0,1,2,4))))
pdf("Fig5_TXNdynamics_n1520.pdf",height=6,width=4)
...
tmp=tmp%>%rename(`0`=PRO0,`1`=PRO1,`2`=PRO2,`4`=PRO4)
pdf("F5_RNAdynamicsBYcluster_n1520_linecol8_2.pdf",height=8,width=2.5)
tmp2=tmp%>%gather("time","txn",-PASid,-newcl,-hgnc)%>%
  mutate(time=as.numeric(time))%>%group_by(PASid)%>%ungroup()
ggplot(tmp2,aes(time,txn))+
  geom_line(aes(col=as.factor(newcl),group=PASid,alpha=0.2))+facet_grid(newcl~.,scales="fixed",switch="both")+
