---
title: "Step2_FilterOutgeneswith local PASswitch"
author: "Yeonui+Kwak"
date: "8/13/2020"
...
```{R}
getwd()
setwd("/Users/yeonuikwak/Desktop/Publication_Mar/Final_Fig4/Rcode/step2")
...
```{R}
head(final)
custom=final%>%separate("newid",c("id","cpspos"),sep=":",remove=F)%>%
  #mutate(refpos=as.numeric(refpos))%>%
...
head(custom2)
write.table(custom2,"custom.n9658.6.15.2020.final.PAL.table.txt",col.names = T,row.names = F,quote=F,sep="\t")
...
newCPSid=id%>%group_by(hgnc,ALE)%>%summarise(t=distance.test(cps=cpspos,id=tandem.new))%>%
...
head(id)
write.table(id,"temporal3pseq_n9567.ALE.tandem.tandemcluster.txt",col.names = T,row.names = F,sep="\t",quote=F)
str(custom2)
...
id.multiple.tandem%>%ungroup()%>%summarise(n_distinct(cluster)) #2374
...
final%>%filter(fdr<0.1)%>%filter(abs(pcor)>0.1)%>%ungroup()%>%summarise(n_distinct(cluster)) #27
final%>%filter(fdr<0.1)%>%filter(abs(pcor)>0.1)%>%ungroup()%>%mutate(cor=ifelse(pcor>0.1,"distal","proximal"))%>%group_by(cor)%>%summarise(n_distinct(cluster)) #distal:21,proximal:7
...
write.table(t,"local tandemUTR switch_statistics.txt",col.names = T,row.names = F,sep="\t",quote=F)
...
nrow(custom2)#9658
nrow(final.custom) #9248
custom2%>%ungroup()%>%select(hgnc)%>%arrange(hgnc)%>%distinct%>%nrow()
final.custom%>%ungroup()%>%select(hgnc)%>%arrange(hgnc)%>%distinct%>%nrow()
...
#result=#
PAL.switch.list
id<-read.table("temporal3pseq_n9567.ALE.tandem.tandemcluster.txt",header=T,stringsAsFactors = F)
